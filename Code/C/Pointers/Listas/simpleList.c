...
void insert_node_pos(miNodo *llist,int Valor); //Insertará un nodo en la posicion correspondiente de forma que esté ordenado
...
void display_list(miNodo *llist) {
...
void append_node(miNodo *llist, int num) {
	while(llist->next != NULL)
		llist = llist->next;
	
	llist->next = (miNodo *)malloc(sizeof(miNodo));
	llist->next->number = num;
	llist->next->next = NULL;
}

void delete_node(miNodo *llist, int num) {
	miNodo *temp;
	temp = (miNodo *)malloc(sizeof(miNodo));
...
int search_value(miNodo *llist, int num) {
	int retval = -1;
	int i = 1;
...
void insert_node_pos(miNodo *llist,int Valor){
